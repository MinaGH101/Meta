...
interface TehranRegionsMapProps {
  ...
  dark: boolean;
  onAreaSelect: (areaId: TehranMapAreaId) => void;
}
...
export function TehranRegionsMap({ activeAreaId, dark, onAreaSelect }: TehranRegionsMapProps) {
  ...
  return (
    <div className="tehran-regions-map" dir="rtl">
      <style>{`
        .tehran-regions-map {
          width: 100%;
          height: 100%;
        ...
        .tehran-regions-map svg {
          width: min(1320px, 91vw);
          height: min(calc(100svh - 122px), 760px);
          display: block;
          overflow: visible;
        }
        ...
        .tehran-region-link:hover .tehran-region-shape {
          fill: ${regionHover};
          stroke: ${ACCENT};
        }

        .tehran-region-link.active .tehran-region-shape {
          fill: ${ACCENT};
          stroke: ${ACCENT};
          filter: drop-shadow(0 3px 7px rgba(189,48,57,0.22));
        }

        .tehran-region-link:focus-visible .tehran-region-shape {
          stroke: ${ACCENT};
          stroke-width: 2.5;
        }

        .tehran-region-number {
          fill: ${labelColor};
        ...
        .tehran-region-link.active .tehran-region-number {
          fill: #ffffff;
        }
        ...
        .tehran-city-link:hover .tehran-city-label,
        .tehran-city-link:focus-visible .tehran-city-label,
        .tehran-city-link.active .tehran-city-label {
          fill: ${ACCENT};
        }
      ...
      <svg
        viewBox="0 -48 760.7 490.8"
        role="img"
        aria-label="نقشه تعاملی مناطق بیست‌ودوگانه تهران"
      ...
      >
        ...
        <g
          role="link"
          tabIndex={0}
          className={`tehran-city-link${activeAreaId === "lavasan" ? " active" : ""}`}
          aria-label="مشاهده پروژه‌های لواسان"
          onClick={() => selectArea("lavasan")}
          onKeyDown={event => handleAreaKeyDown(event, "lavasan")}
        >
        ...
        {REGION_PATHS.map(region => {
          const active = activeAreaId === region.id;
          ...
          return (
            <g
              ...
              role="link"
              tabIndex={0}
              className={`tehran-region-link${active ? " active" : ""}`}
              aria-label={`مشاهده پروژه‌های منطقه ${toPersianNumber(region.id)}`}
              onClick={() => selectArea(region.id)}
              onKeyDown={event => handleAreaKeyDown(event, region.id)}
            >
              <title>{`منطقه ${toPersianNumber(region.id)}`}</title>
              <path className="tehran-region-shape" d={region.d} />
              <text
                x={region.labelX}
                y={region.labelY}
                className="tehran-region-number"
                dominantBaseline="middle"
              >
                {toPersianNumber(region.id)}
              </text>
            </g>
          );
        })}
