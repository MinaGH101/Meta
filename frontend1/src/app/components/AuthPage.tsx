...
export function AuthPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.phone || !form.password) { setError("لطفاً شماره موبایل و رمز عبور را وارد کنید"); return; }
    setError("");
    try {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.phone || !form.password) { setError("لطفاً تمام فیلدهای الزامی را پر کنید"); return; }
        ...
        >
          ...
          {tab === "register" && (
            <form onSubmit={handleRegister} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label style={{ fontSize: "0.78rem", color: muted }}>ایمیل (اختیاری)</label>
              ...
              <div className="flex flex-col gap-1.5">
                <label style={{ fontSize: "0.78rem", color: muted }}>شماره موبایل *</label>
                <input type="tel" style={inputStyle} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="۰۹۱۲۳۴۵۶۷۸۹" />
              </div>
              <div className="flex flex-col gap-1.5">
                <label style={{ fontSize: "0.78rem", color: muted }}>رمز عبور *</label>
                <div className="relative">
