...
export function ContactPage() {
  ...
  useEffect(() => {
    apiFetch<PageContent>("/page-content/contact").then(setPage).catch(() => undefined);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSent(true); setForm({ name: user?.full_name || "", email: user?.email || "", subject: "", message: "" });
  ...
  const contactInfo = [
    { icon: <MapPin size={18} />, label: "آدرس", value: content.address || "تهران، ایران" },
    { icon: <Phone size={18} />, label: "تلفن", value: content.phone || "021-26377334" },
    { icon: <Mail size={18} />, label: "تلفن همراه", value: content.phone || "09121750837 - 09125969447" },
    { icon: <Clock size={18} />, label: "ساعات کاری", value: content.hours || "شنبه تا چهارشنبه، ۸ تا ۱۷" },
  ];
  ...
  return (
    <AppShell>
      ...
      >
        {/* Header */}
        <div className="w-full mx-auto px-5 md:px-10 py-12" style={{ maxWidth: 1180 }}>
          <div className="flex items-center gap-3 mb-3">
            <div style={{ width: 3, height: 36, background: ACCENT, borderRadius: 2 }} />
            <h1 style={{ fontSize: "clamp(1.8rem, 4vw, 2.8rem)", fontWeight: 700, margin: 0 }}>{page?.title || "تماس با ما"}</h1>
          </div>
        ...
        {/* Content */}
        <div className="flex flex-col md:flex-row gap-0 flex-1 w-full mx-auto" style={{ maxWidth: 1180 }}>
          {/* Info panel */}
          <div
          ...
          >
            ...
            <div
              ...
              style={{ height: 140, background: card, border: "none" }}
            >
              <iframe
                title="موقعیت هلدینگ متا"
                src="https://www.google.com/maps?q=35.81865863196015,51.61413930234827&z=16&output=embed"
                width="100%"
                height="100%"
          ...
          <div className="flex flex-col p-10 flex-1">
            ...
            {error && <div style={{ color: "#ff8f98", marginBottom: 12 }}>{error}</div>}
            {sent && (
              <div
